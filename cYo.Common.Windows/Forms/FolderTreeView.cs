...
namespace cYo.Common.Windows.Forms
{
    public class FolderTreeView : TreeViewEx
    {
        ...
        public static class ShellOperations
        {
            ...
            /// <summary>
            /// Renames the folder on disk and updates the node's Tag to reflect the new path
            /// </summary>
            /// <returns><see cref="bool"/> that determines if it was sucessfull</returns>
            public static bool RenameFolderPath(TreeNode tn, string newPath)
            {
                string oldPath = GetFilePath(tn);
                try
                {
                    // TODO: See if using Native Methods to rename the folder is warranted
                    if (!Directory.Exists(oldPath) || Directory.Exists(newPath))
                        return false;

                    Directory.Move(oldPath, newPath); // Rename folder
                    tn.Tag = new ShellFolder(newPath); // Update the node's tag to reflect the new path
                    return true;
                }
                catch (Exception ex)
                {
                    System.Diagnostics.Debug.WriteLine($"Failed to rename folder:\n{ex.Message}");
                    return false;
                }
            }
        ...
        /// <summary>
        /// Let's rename the folder on disk when the user finishes editing the label of a node<br/><br/>
        /// Checks if the new label is valid and if the folder is not a special folder (Desktop, Documents, etc.) before attempting to rename.<br/>
        /// If the rename fails for any reason, the edit is cancelled which will revert the label back to the old name.
        /// </summary>
        protected override void OnAfterLabelEdit(NodeLabelEditEventArgs e)
        {
            base.OnAfterLabelEdit(e);

            string oldPath = ShellOperations.GetFilePath(e.Node);
            var specialFolder = FindSpecialFolder(oldPath); // To ensure that we don't rename special folders like Desktop, Documents, etc.
            if (e.Label == null || e.Node == null || specialFolder != null)
            {
                e.CancelEdit = true;
                return;
            }
            ...
            if (!ShellOperations.RenameFolderPath(e.Node, newPath)) // This will rename & update the node's Tag to the new path. 
                e.CancelEdit = true; // If it fails, we cancel the edit which will revert the label back to the old name
        }
        ...
        // Source - https://stackoverflow.com/a/72040338
